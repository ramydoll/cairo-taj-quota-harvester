...
async function harvestQuota() {
  ...
  try {
    ...
    if (postLoginState === 'captcha') {
      ...
      //
      // Strategy per round:
      //   1. Run ALL 18 filters → collect every 5-char OCR result
      //   2. Vote: group by lowercased value, count occurrences (weight: group A=3, B=2, C=1, D=1)
      //   3. Submit top-2 by weighted vote, cycling orig/UPPER/lower case per round
      //   4. colorOnly/colorStrong getting 5-char → tried with all 3 case variants FIRST
      //   5. Reject any OCR result < 5 chars silently (logged)
      ...
      for (let round = 1; round <= 12 && !captchaSolved; round++) {
        ...
        if (round > 1) {
          ...
          if (!modalFound) {
            ...
            try {
              await page.evaluate(() => {
                // Clear and re-fill username
                const u = document.querySelector('#login_loginid_input_01');
                if (u) { u.focus(); u.select(); }
              });
              await sleep(300);
              // Re-type password (most reliable trigger for captcha re-appearance)
          ...
          if (captchaSolved) break;
          await sleep(1200); // Let new image fully render
        }
        ...
        try {
          ...
          // Try all 3 case variants immediately if these get a 5-char result
          ...
          const votes = {}; // key=lowercased, value={ weight, original }
          for (const filter of ALL_FILTERS) {
            if (['colorOnly','colorStrong'].includes(filter)) continue; // already tried in phase 1
