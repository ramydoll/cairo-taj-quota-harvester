...
async function harvestQuota() {
  ...
  try {
    ...
    if (postLoginState === 'captcha') {
      ...
      async function canvasProcess(imgHandle, filter) {
        return await page.evaluate((imgEl, f) => {
          ...
          for (let i = 0; i < d.length; i += 4) {
            ...
            let keep = false;
            if      (f === 'redOnly')        { keep = r > 100 && (r - g) > 30 && (r - b) > 20; }
            else if (f === 'redStrict')      { keep = r > 140 && (r - g) > 50 && (r - b) > 40; }
            else if (f === 'redWide')        { keep = r > 80 && r > g * 1.2 && r > b * 1.15; }
            else if (f === 'darkNoBlue')     { keep = lum < 140 && !(b > r + 20 && b > g + 15); }
            else if (f === 'saturationBoost'){ keep = sat > 0.35 && lum < 210; }
            else if (f === 'warmColors')     { keep = r > g && r > b && lum < 200 && sat > 0.2; }
            else if (f === 'colorOnly')      { keep = sat > 0.25 && lum < 195 && lum > 20; }
            else if (f === 'channelDivide')  { keep = (r / (b + 1)) > 1.4 && lum < 180; }
            else if (f === 'megaRed')        { keep = r > 160 && (r - g) > 60 && (r - b) > 50; }
      ...
      const FILTERS = ['redOnly','redStrict','redWide','darkNoBlue','saturationBoost','warmColors','colorOnly','channelDivide','megaRed','blueInverter','thresh120','thresh160','dilate'];
